...
function testUiHasRequiredTermsCheckbox() {
  ...
  assert.ok(html.includes('id="regTermsBlock"'), 'terms block container present');
  assert.ok(html.includes('terms-acceptance'), 'terms visibility class present');
  assert.ok(html.includes('href="/terms.html"'), 'links to terms page');
  assert.ok(html.includes('required'), 'required attribute present on register form terms control');
  assert.ok(html.includes('name="terms_accepted"'), 'terms_accepted field name');
  assert.ok(/\(required\)/i.test(html), 'required label cue visible to users');
  ...
  assert.ok(dash.includes('terms-acceptance-error'), 'dashboard highlights missing acceptance');
  assert.ok(auth.includes('terms_accepted'), 'auth client posts terms_accepted');
  assert.ok(styles.includes('.field-checkbox.terms-acceptance'), 'styles make terms block visible');
  console.log('ok');
}
