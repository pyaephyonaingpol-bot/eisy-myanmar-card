...
/**
 * Persist a successfully created provider card into Supabase `user_cards`.
 * Saves PAN / CVV / expiry / balance plus provider raw payload in metadata.
 */
async function storeIssuedCard({
  ...
  const row = {
    ...
    metadata: {
      ...
      provider_raw: providerRaw || card.raw_payload || null,
      ...metadata,
    },
...
/**
 * Direct integration helper: POST createcard → normalize → upsert Supabase user_cards.
 * Prefer `issueCardForUser` for production flows (validation + idempotency).
 */
async function createAndPersistKripicardCard({
  userId,
  nameOnCard,
  bin,
  amount,
